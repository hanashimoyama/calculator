...
public class Main {
    public static <string> void main(String[] args) {
        ...
        while (sc.hasNextLine()) {
            ...
            total = Integer.parseInt(formula2.substring(0, index1));//合計をいれる値を作成
            int index2 = operatorsIndexOf(formula2, index1 + 1);
            while (index2 != -1) {
                int value2 = Integer.parseInt(formula2.substring(index1 + 1, index2)); //演算子がまだあるかチェックして見つかった時のループ処理
            ...
            int value2 = Integer.parseInt(formula2.substring(index1 + 1)); //ループ処理をぬけて演算子がこれ以上無いときの処理
            switch (formula2.charAt(index1)) {
            ...
            System.out.print("式:");
                ...
                public static int operatorsIndexOf (String f,int start){
                    int operatorPlusIndex = f.indexOf("+",start); //文字列にそれぞれの演算子があるかチェック
